...
from cvcore.losses import LabelSmoothingCrossEntropy, SoftCrossEntropy
...
def main(args, cfg):
    ...
    if cfg.LOSS.NAME == "ce":
        ...
        soft_criterion = SoftCrossEntropy().cuda()
    ...
    if args.mode in ("train", "valid"):
        ...
        valid_loader = make_dataloader(
            cfg, "valid", valid_df["image_id"].values, valid_df["label"].values, valid_df.iloc[:,2:-1].values)
        if args.mode == "train":
    ...
    if args.mode == "train":
        for epoch in range(start_epoch, cfg.TRAIN.EPOCHES[-1]):
            ...
            train_loop(logger.info, cfg, model,
                       ...
                       scheduler, epoch, scaler, soft_criterion)
